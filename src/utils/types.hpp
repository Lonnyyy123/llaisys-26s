...
inline fp16_t _f32_to_f16(float val) {
    ...
    if (exponent >= 16) {
        ...
        return fp16_t{static_cast<uint16_t>(sign | ((exponent + 15) << 10) |
                                             (mantissa >> 13))};
    } else if (exponent >= -24) {
        mantissa |= 0x800000;
...
inline float _bf16_to_f32(bf16_t val) {
    const uint32_t bits32 = static_cast<uint32_t>(val._v) << 16;
    float out;
...
inline bf16_t _f32_to_bf16(float val) {
    ...
    return bf16_t{static_cast<uint16_t>((bits32 + rounding_bias) >> 16)};
}
